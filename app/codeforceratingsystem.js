...
export class CodeforcesRatingCalculator2 {
  /**
  ...
  calculateRatingChanges(previousRatings, standingsRows) {
    const contestants = standingsRows.map((standingsRow) => {
      ...
      return new Contestant(
        party,
        rank,
        standingsRow.getPoints(),
        previousRatings.get(party)
      );
    });
  ...
  process(contestants) {
    ...
    contestants = contestants.map((contestant) => {
      ...
      contestant.needRating = this.getRatingToRank(
        contestants,
        contestant,
        midRank
      );
      ...
      for (let j = i + 1; j < contestants.length; j++) {
        ...
        if (contestants[i].rating < contestants[j].rating) {
          ...
          if (contestants[i].delta < contestants[j].delta) {
            throw new Error(
              "Second rating invariant failed" +
                contestants[i].party.username +
                " vs. " +
                contestants[j].party.username +
                "."
...
export function getNewRatings(contestants) {
